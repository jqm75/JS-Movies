...
function getMoviesFromDirector(array, director) {
  //El método filter también crea un nuevo Array.
...
function hoursToMinutes(array) {
  const moviesInMinutes = array.map((movie) => {
    ...
    let hours =
      Number(newArrayDuration[0]) > 0 ? Number(newArrayDuration[0]) * 60 : 0;
    let minutes =
      newArrayDuration.length > 2
        ? Number(newArrayDuration[1] + newArrayDuration[2])
        : 0;
